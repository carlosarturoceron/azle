...
class AsyncHook {
    constructor({ init, before, after, destroy, promiseResolve }) {
        ...
        if (
            ...
            typeof promiseResolve !== 'function'
        )
    ...
    enable() {
        ...
        hook_fields[kTotals] += hook_fields[kDestroy] +=
            +!!this[destroy_symbol];
...
class AsyncResource {
    ...
    static bind(fn, type, thisArg) {
        type = type || fn.name;
        return new AsyncResource(type || 'bound-anonymous-fn').bind(
            fn,
            thisArg
        );
    }
...
class AsyncLocalStorage {
    ...
    disable() {
        if (this.enabled) {
            ...
            ArrayPrototypeSplice(
                storageList,
                ArrayPrototypeIndexOf(storageList, this),
                1
            );
...
module.exports = {
    ...
    triggerAsyncId,
    executionAsyncResource,
    asyncWrapProviders: ObjectFreeze({
        __proto__: null,
        ...asyncWrap.Providers
    }),
    // Embedder API
