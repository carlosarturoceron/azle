...
class Interface extends InterfaceConstructor {
    ...
    [kNormalWrite](b) {
        ...
        if (newPartContainsEnding !== null) {
            ...
            const indexes = [
                ...
                lineEnding.lastIndex
            ];
            let nextMatch;
            while (
                (nextMatch = RegExpPrototypeExec(lineEnding, string)) !== null
            ) {
                ArrayPrototypePush(
                    indexes,
                    nextMatch.index,
            ...
            this[kLine_buffer] = StringPrototypeSlice(
                string,
                indexes[lastIndex]
            );
    ...
    [kTabCompleter](lastKeypressWasTab, { 0: completions, 1: completeOn }) {
        ...
        if (
            ...
            this[kInsertString](
                StringPrototypeSlice(prefix, completeOn.length)
            );
            return;
    ...
    [kGetDisplayPos](str) {
        ...
        for (const char of new SafeStringIterator(str)) {
            ...
            const width = getStringWidth(
                char,
                false /* stripVTControlCharacters */
            );
            if (width === 0 || width === 1) {
                offset += width;
            } else {
            ...
            switch (key.name) {
                ...
                default:
                    if (typeof s === 'string' && s) {
                        ...
                        let lastIndex = 0;
                        while (
                            (nextMatch = RegExpPrototypeExec(lineEnding, s)) !==
                            null
                        ) {
                            this[kInsertString](
                                StringPrototypeSlice(
    ...
    [SymbolAsyncIterator]() {
        if (this[kLineObjectStream] === undefined) {
            kFirstEventParam ??=
                require('internal/events/symbols').kFirstEventParam;
