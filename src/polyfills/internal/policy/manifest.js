        ...
        if (
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (StringPrototypeEndsWith(key, '*')) {
                    ...
                    patterns.push([target.slice(0, -1), [prefix, suffix]]);
                }
            }
        ...
        if (dependencies !== undefined && typeof dependencies === 'object') {
            ...
            if (normalizedSpecifier in dependencies === true) {
                ...
                if (parsedURLs && parsedURLs.has(to)) {
                    ...
                    RegExpPrototypeExec(kRelativeURLStringPattern, to) !== null
                ) {
...
class Manifest {
    ...
    constructor(obj, manifestHREF) {
        ...
        const jsonScopesEntries = ObjectEntries(
            obj.scopes ?? { __proto__: null }
        );
        ...
        for (let i = 0; i < jsonResourcesEntries.length; i++) {
            ...
            if (typeof integrity !== 'undefined') {
                debug(
                    'Manifest contains integrity for resource %s',
                    originalHREF
                );
        ...
        if (integrities.has(href)) {
            ...
            if (ArrayIsArray(integrityEntries)) {
                ...
                for (let i = 0; i < integrityEntries.length; i++) {
                    ...
                    const mismatchedIntegrity = internalVerifyIntegrity(
                        algorithm,
                        content,
                        expected
                    );
            ...
            if (integrityEntries !== kCascade) {
                const error = new ERR_MANIFEST_ASSERT_INTEGRITY(
                    url,
                    realIntegrities
                );
                this.#reaction(error);
...
function canonicalizeSpecifier(specifier, base) {
    try {
        if (
            RegExpPrototypeExec(kRelativeURLStringPattern, specifier) !== null
        ) {
...
const emptyOrProtocolOrResolve = (resourceHREF, base) => {
    ...
    if (StringPrototypeEndsWith(resourceHREF, ':')) {
        ...
        if (
            RegExpPrototypeExec(/^[a-zA-Z][a-zA-Z+\-.]*:$/, resourceHREF) !==
            null
        ) {
            return resourceHREF;
        }
...
const resolve = (originalHREF, base) => {
    ...
    if (parsedURLs.has(originalHREF)) {
        ...
        RegExpPrototypeExec(kRelativeURLStringPattern, originalHREF) !== null
    ) {
