...
function setupUndici() {
    ...
    if (!getOptionValue('--no-experimental-fetch')) {
        ...
        internalBinding('wasm_web_api').setImplementation(
            (streamState, source) => {
                require('internal/wasm_web_api').wasmStreamingCallback(
                    streamState,
                    source
                );
            }
...
function setupNavigator() {
    if (
        getEmbedderOptions().noBrowserGlobals ||
...
function setupWebCrypto() {
    ...
    if (internalBinding('config').hasOpenSSL) {
        ...
        exposeLazyInterfaces(globalThis, 'internal/crypto/webcrypto', [
            'Crypto',
            'CryptoKey',
            'SubtleCrypto'
        ]);
...
function setupCodeCoverage() {
    ...
    if (
        ...
        process.env.NODE_V8_COVERAGE = setupCoverageHooks(
            process.env.NODE_V8_COVERAGE
        );
    }
}
...
function setupDebugEnv() {
    require('internal/util/debuglog').initializeDebugEnv(
    ...
    if (pendingDeprecation) {
        process.binding = deprecate(
            ...
            'process.binding() is deprecated. ' +
                'Please use public APIs instead.',
            'DEP0111'
        );
...
function initializePermission() {
    ...
    if (experimentalPermission) {
        ...
        process.emitWarning(
            'Permission is an experimental feature',
            'ExperimentalWarning'
        );
        const { has, deny } = require('internal/process/permission');
        const warnFlags = ['--allow-child-process', '--allow-worker'];
        ...
        const warnCommaFlags = ['--allow-fs-read', '--allow-fs-write'];
...
function readPolicyFromDisk() {
    ...
    if (experimentalPolicy) {
        ...
        const experimentalPolicyIntegrity =
        ...
        if (experimentalPolicyIntegrity) {
            ...
            if (!foundMatch) {
                throw new ERR_MANIFEST_ASSERT_INTEGRITY(
                    manifestURL,
                    realIntegrities
                );
            }
...
function getHeapSnapshotFilename(diagnosticDir) {
    ...
    const fileSequence = (++sequenceNumOfheapSnapshot)
        .toString()
        .padStart(3, '0');
