...
function normalizeAlgorithm(algorithm, op) {
    ...
    for (let i = 0; i < dictKeys.length; i++) {
        ...
        if (!ObjectPrototypeHasOwnProperty(dict, member)) continue;
        ...
        if (idlType === 'BufferSource' && idlValue) {
            ...
            normalizedAlgorithm[member] = normalizeAlgorithm(
                idlValue,
                'digest'
            );
...
function hasAnyNotIn(set, checks) {
    for (const s of set) if (!ArrayPrototypeIncludes(checks, s)) return true;
    return false;
}
...
function getUsagesUnion(usageSet, ...usages) {
    ...
    for (let n = 0; n < usages.length; n++) {
        if (usageSet.has(usages[n])) ArrayPrototypePush(newset, usages[n]);
    }
    return newset;
}
