...
async function generateKey(algorithm, extractable, keyUsages) {
    ...
    switch (algorithm.name) {
        ...
        case 'X448':
            resultType = 'CryptoKeyPair';
            result = await require('internal/crypto/cfrg').cfrgGenerateKey(
                algorithm,
                extractable,
                keyUsages
            );
        ...
        case 'ECDH':
            resultType = 'CryptoKeyPair';
            result = await require('internal/crypto/ec').ecGenerateKey(
                algorithm,
                extractable,
                keyUsages
            );
        ...
        case 'HMAC':
            ...
            result = await require('internal/crypto/mac').hmacGenerateKey(
                algorithm,
                extractable,
                keyUsages
            );
    ...
    if (
        ...
        (resultType === 'CryptoKeyPair' &&
            result.privateKey.usages.length === 0)
    ) {
        throw lazyDOMException(
...
async function deriveBits(algorithm, baseKey, length) {
    ...
    switch (algorithm.name) {
        ...
        // Fall through
        case 'ECDH':
            return require('internal/crypto/diffiehellman').ecdhDeriveBits(
                algorithm,
                baseKey,
                length
            );
        case 'HKDF':
            return require('internal/crypto/hkdf').hkdfDeriveBits(
                algorithm,
                baseKey,
                length
            );
        case 'PBKDF2':
            return require('internal/crypto/pbkdf2').pbkdf2DeriveBits(
                algorithm,
                baseKey,
                length
            );
...
async function deriveKey(
    ...
    switch (algorithm.name) {
        ...
        case 'ECDH':
            bits =
                await require('internal/crypto/diffiehellman').ecdhDeriveBits(
                    algorithm,
                    baseKey,
                    length
                );
            break;
        case 'HKDF':
            bits = await require('internal/crypto/hkdf').hkdfDeriveBits(
                algorithm,
                baseKey,
                length
            );
            break;
        case 'PBKDF2':
            bits = await require('internal/crypto/pbkdf2').pbkdf2DeriveBits(
                algorithm,
                baseKey,
                length
            );
...
async function exportKeySpki(key) {
    switch (key.algorithm.name) {
        ...
        case 'ECDH':
            if (key.type === 'public') {
                return require('internal/crypto/ec').ecExportKey(
                    key,
                    kWebCryptoKeyFormatSPKI
                );
            }
...
async function exportKeyJWK(key) {
    ...
    switch (key.algorithm.name) {
        ...
        // Fall through
        case 'AES-KW':
            jwk.alg = require('internal/crypto/aes').getAlgorithmName(
                key.algorithm.name,
...
async function importKey(format, keyData, algorithm, extractable, keyUsages) {
    ...
    switch (algorithm.name) {
        ...
        // Fall through
        case 'ECDH':
            result = await require('internal/crypto/ec').ecImportKey(
                format,
                keyData,
                algorithm,
                extractable,
...
async function wrapKey(format, key, wrappingKey, algorithm) {
    ...
    if (format === 'jwk') {
        ...
        if (algorithm.name === 'AES-KW' && raw.length % 8 !== 0) {
            keyData = ec.encode(
                raw + StringPrototypeRepeat(' ', 8 - (raw.length % 8))
            );
        } else {
...
function signVerify(algorithm, key, data, signature) {
    ...
    switch (algorithm.name) {
        ...
        case 'ECDSA':
            return require('internal/crypto/ec').ecdsaSignVerify(
                key,
                data,
                algorithm,
                signature
