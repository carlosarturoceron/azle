...
class ReadableStream {
    ...
    getReader(options = kEmptyObject) {
        ...
        validateObject(
            options,
            'options',
            kValidateObjectAllowNullable | kValidateObjectAllowFunction
        );
    ...
    pipeThrough(transform, options = kEmptyObject) {
        ...
        if (isReadableStreamLocked(this))
            throw new ERR_INVALID_STATE.TypeError(
                'The ReadableStream is locked'
            );
        if (isWritableStreamLocked(writable))
            throw new ERR_INVALID_STATE.TypeError(
                'The WritableStream is locked'
            );
    ...
    values(options = kEmptyObject) {
        ...
        function nextSteps() {
            ...
            readableStreamDefaultReaderRead(
                reader,
                new ReadableStreamAsyncIteratorReadRequest(
                    reader,
                    state,
                    promise
                )
...
class ReadableStreamBYOBRequest {
    ...
    respond(bytesWritten) {
        ...
        const viewBufferByteLength =
            ArrayBufferPrototypeGetByteLength(viewBuffer);
...
ObjectDefineProperties(ReadableStreamBYOBRequest.prototype, {
    ...
    [SymbolToStringTag]: getNonWritablePropertyDescriptor(
...
class ReadableStreamBYOBReader {
    ...
    read(view) {
        if (!isReadableStreamBYOBReader(this))
            return PromiseReject(
                new ERR_INVALID_THIS('ReadableStreamBYOBReader')
            );
...
class ReadableByteStreamController {
    ...
    get byobRequest() {
        ...
        if (
            ...
            this[kState].byobRequest = createReadableStreamBYOBRequest(
                this,
                view
            );
        }
    ...
    enqueue(chunk) {
        ...
        const chunkBufferByteLength =
            ArrayBufferPrototypeGetByteLength(chunkBuffer);
...
function readableStreamFromIterable(iterable) {
    ...
    async function cancelAlgorithm(reason) {
        ...
        const returnResult = FunctionPrototypeCall(
            returnMethod,
            iterator,
            reason
        );
...
function readableStreamPipeTo(
    ...
    function watchErrored(stream, promise, action) {
        ...
        else PromisePrototypeThen(promise, undefined, action);
    }
...
function readableByteStreamTee(stream) {
    ...
    function pullWithDefaultReader() {
        ...
        const readRequest = {
            ...
            [kClose]() {
                ...
                if (
                    branch1[kState].controller[kState].pendingPullIntos.length >
                    0
                ) {
                ...
                if (
                    branch2[kState].controller[kState].pendingPullIntos.length >
                    0
                ) {
                    readableByteStreamControllerRespond(
                        branch2[kState].controller,
        ...
        const readIntoRequest = {
            [kChunk](chunk) {
                queueMicrotask(() => {
                    ...
                    readAgainForBranch2 = false;
                    const byobCanceled =
                        forBranch2 === true ? canceled2 : canceled1;
                    const otherCanceled =
                        forBranch2 === false ? canceled2 : canceled1;
            ...
            [kClose](chunk) {
                ...
                if (chunk !== undefined) {
                    ...
                    if (
                        !otherCanceled &&
                        otherBranch[kState].controller[kState].pendingPullIntos
                            .length > 0
                    ) {
                        readableByteStreamControllerRespond(
                            otherBranch[kState].controller,
    ...
    function cancel1Algorithm(reason) {
        ...
        if (canceled2) {
            cancelDeferred.resolve(
                readableStreamCancel(stream, [reason1, reason2])
            );
        }
...
function readableStreamFulfillReadIntoRequest(stream, chunk, done) {
    ...
    const readIntoRequest = ArrayPrototypeShift(
        reader[kState].readIntoRequests
    );
