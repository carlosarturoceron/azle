...
const legacyWrapperList = new SafeSet(['natives', 'util']);

// The code bellow assumes that the two lists must not contain any modules
// beginning with "internal/".
// Modules that can only be imported via the node: scheme.
const schemelessBlockList = new SafeSet(['test', 'test/reporters']);
...
const { builtinIds, compileFunction, setInternalLoaders } =
...
const publicBuiltinIds = builtinIds.filter(
    (id) =>
...
const internalBuiltinIds = builtinIds.filter(
    (id) => StringPrototypeStartsWith(id, 'internal/') && id !== selfId
);
...
let canBeRequiredByUsersWithoutSchemeList = new SafeSet(
    publicBuiltinIds.filter((id) => !schemelessBlockList.has(id))
);
...
class BuiltinModule {
    ...
    static setRealmAllowRequireByUsers(ids) {
        canBeRequiredByUsersList = new SafeSet(
            ArrayPrototypeFilter(ids, (id) =>
                ArrayPrototypeIncludes(publicBuiltinIds, id)
            )
        );
        canBeRequiredByUsersWithoutSchemeList = new SafeSet(
            ArrayPrototypeFilter(ids, (id) => !schemelessBlockList.has(id))
        );
    }
