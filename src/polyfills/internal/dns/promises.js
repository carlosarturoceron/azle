...
const kPerfHooksDnsLookupResolveContext = Symbol(
    'kPerfHooksDnsLookupResolveContext'
);
...
function createLookupServicePromise(hostname, port) {
    return new Promise((resolve, reject) => {
        ...
        if (err) reject(new DNSException(err, 'getnameinfo', hostname));
        else if (hasObserver('dns')) {
...
function createResolverPromise(resolver, bindingName, hostname, ttl) {
    return new Promise((resolve, reject) => {
        ...
        if (err) reject(new DNSException(err, bindingName, hostname));
        else if (hasObserver('dns')) {
