...
function serializeError(error) {
    ...
    if (typeof error === 'symbol') {
        return Buffer.from(
            StringFromCharCode(kInspectedSymbol) + inspect(error),
            'utf8'
        );
    }
    try {
    ...
    try {
        if (error != null && customInspectSymbol in error) {
            return Buffer.from(
                StringFromCharCode(kCustomInspectedObject) + inspect(error),
                'utf8'
            );
        }
    } catch {
    ...
    return Buffer.from(
        StringFromCharCode(kInspectedError) + inspect(error),
        'utf8'
    );
}
...
function fromBuffer(error) {
    return Buffer.from(
        TypedArrayPrototypeGetBuffer(error),
