'use strict';
const { Error, StringPrototypeSplit } = primordials;
const {
...
const {
    privateSymbols: { transfer_mode_private_symbol },
    constants: { kDisallowCloneAndTransfer, kTransferable, kCloneable }
...
function setup() {
    ...
    setDeserializerCreateObjectFunction((deserializeInfo) => {
        const { 0: module, 1: ctor } = StringPrototypeSplit(
            deserializeInfo,
            ':'
        );
