...
function deserializePackageJSON(path, contents, checkIntegrity = false) {
    ...
    return {
        ...
        get imports() {
            const value = requiresJSONParse(plainImports)
                ? JSONParse(plainImports)
                : plainImports;
            ObjectDefineProperty(this, 'imports', { __proto__: null, value });
            return this.imports;
        },
        get exports() {
            const value = requiresJSONParse(plainExports)
                ? JSONParse(plainExports)
                : plainExports;
...
function getNearestParentPackageJSON(checkPath) {
    ...
    const data = deserializePackageJSON(
        checkPath,
        result,
        true /* checkIntegrity */
    );
    ...
    const path = StringPrototypeSlice(
        data.pjsonPath,
        0,
        StringPrototypeLastIndexOf(data.pjsonPath, sep)
    );
