...
class Hooks {
    ...
    addCustomLoader(url, exports, data) {
        ...
        if (resolve) {
            ...
            ArrayPrototypePush(this.#chains.resolve, {
                __proto__: null,
                fn: resolve,
                url,
                next
        ...
        if (load) {
            ...
            ArrayPrototypePush(this.#chains.load, {
                __proto__: null,
                fn: load,
                url,
                next
    ...
    async resolve(
        ...
        if (
            !('importAttributes' in resolution) &&
            'importAssertions' in resolution
        ) {
            emitImportAssertionWarning();
...
class HooksProxy {
    ...
    async makeAsyncRequest(method, transferList, ...args) {
        ...
        do {
            ...
            await AtomicsWaitAsync(
                this.#lock,
                WORKER_TO_MAIN_THREAD_NOTIFICATION,
                this.#workerNotificationLastId
            ).value;
            this.#workerNotificationLastId = AtomicsLoad(
                this.#lock,
                WORKER_TO_MAIN_THREAD_NOTIFICATION
            );
...
function nextHookFactory(current, meta, { validateArgs, validateOutput }) {
    ...
    return ObjectDefineProperty(
        async (arg0 = undefined, context) => {
            ...
            validateArgs(
                `${meta.hookErrIdentifier} hook's ${nextHookName}()`,
                arg0,
                context
            );
