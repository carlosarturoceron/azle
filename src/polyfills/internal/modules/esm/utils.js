...
const { ArrayIsArray, SafeSet, SafeWeakMap, Symbol, ObjectFreeze } =
    primordials;

const {
    privateSymbols: { host_defined_option_symbol }
} = internalBinding('util');
const { default_host_defined_options, vm_dynamic_import_missing_flag } =
...
function registerModule(referrer, registry) {
    ...
    if (
        idSymbol === default_host_defined_options ||
...
function initializeImportMetaObject(symbol, meta) {
    if (moduleRegistries.has(symbol)) {
        const { initializeImportMeta, callbackReferrer } =
 ...
 */
async function importModuleDynamicallyCallback(
    referrerSymbol,
    specifier,
    attributes
) {
    if (moduleRegistries.has(referrerSymbol)) {
        const { importModuleDynamically, callbackReferrer } =
