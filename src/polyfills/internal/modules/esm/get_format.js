...
function extname(url) {
    ...
    for (let i = pathname.length - 1; i > 0; i--) {
        switch (StringPrototypeCharCodeAt(pathname, i)) {
            ...
            case DOT_CODE:
                return StringPrototypeCharCodeAt(pathname, i - 1) === SLASH_CODE
                    ? ''
                    : StringPrototypeSlice(pathname, i);
        }
    }
    return '';
}
        ...
        switch (getOptionValue('--experimental-default-type')) {
            ...
            default: {
                ...
                if (getOptionValue('--experimental-detect-module')) {
                    return source
                        ? containsModuleSyntax(`${source}`, fileURLToPath(url))
                            ? 'module'
                            : 'commonjs'
                        : null;
                }
