...
class ModuleJob {
    ...
    constructor(
        ...
        this.modulePromise = ReflectApply(moduleProvider, loader, [
            url,
            isMain
        ]);
        ...
        const link = async () => {
            ...
            const promises = this.module.link(async (specifier, attributes) => {
                const job = await this.loader.getModuleJob(
                    specifier,
                    url,
                    attributes
                );
    ...
    async _instantiate() {
        ...
        const addJobsToDependencyGraph = async (moduleJob) => {
            ...
            return SafePromiseAllReturnVoid(
                dependencyJobs,
        ...
        try {
            ...
            if (
                ...
                if (format === 'commonjs') {
                    ...
                    const oneLineNamedImports = RegExpPrototypeExec(
                        /{.*}/,
                        importStatement
                    );
        ...
        try {
            ...
            if (
                ...
                const packageConfig =
                    ...
                    require('internal/modules/package_json_reader').getPackageScopeConfig(
                        this.module.url
                    );
