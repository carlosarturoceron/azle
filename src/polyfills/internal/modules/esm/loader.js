...
const { ERR_REQUIRE_ESM, ERR_UNKNOWN_MODULE_FORMAT } =
...
class ModuleLoader {
    ...
    async getModuleJob(specifier, parentURL, importAttributes) {
        ...
        return this.getJobFromResolveResult(
            resolveResult,
            parentURL,
            importAttributes
        );
    ...
    getModuleJobSync(specifier, parentURL, importAttributes) {
        const resolveResult = this.resolveSync(
            specifier,
            parentURL,
            importAttributes
        );
    ...
    getJobFromResolveResult(resolveResult, parentURL, importAttributes, sync) {
        ...
        const resolvedImportAttributes =
            resolveResult.importAttributes ?? importAttributes;
    ...
    #createModuleJob(url, importAttributes, parentURL, format, sync) {
        const callTranslator = (
            { format: finalFormat, responseURL, source },
            isMain
        ) => {
        ...
        const moduleProvider = sync
            ? (url, isMain) =>
                  callTranslator(this.loadSync(url, context), isMain)
            : async (url, isMain) =>
                  callTranslator(await this.load(url, context), isMain);
    ...
    async import(specifier, parentURL, importAttributes) {
        const moduleJob = await this.getModuleJob(
            specifier,
            parentURL,
            importAttributes
        );
    ...
    resolve(originalSpecifier, parentURL, importAttributes) {
        if (this.#customizations) {
            return this.#customizations.resolve(
                originalSpecifier,
                parentURL,
                importAttributes
            );
        }
        const requestKey = this.#resolveCache.serializeKey(
            originalSpecifier,
            importAttributes
        );
        ...
        const result = this.defaultResolve(
            originalSpecifier,
            parentURL,
            importAttributes
        );
    ...
    resolveSync(originalSpecifier, parentURL, importAttributes) {
        if (this.#customizations) {
            return this.#customizations.resolveSync(
                originalSpecifier,
                parentURL,
                importAttributes
            );
    ...
    defaultResolve(originalSpecifier, parentURL, importAttributes) {
        defaultResolve ??=
            require('internal/modules/esm/resolve').defaultResolve;
    ...
    validateLoadResult(url, format) {
        if (format == null) {
            require('internal/modules/esm/load').throwUnknownModuleFormat(
                url,
                format
            );
        }
    ...
    importMetaInitialize(meta, context) {
        if (this.#customizations) {
            return this.#customizations.importMetaInitialize(
                meta,
                context,
                this
            );
        ...
        importMetaInitializer ??=
            require('internal/modules/esm/initialize_import_meta').initializeImportMeta;
