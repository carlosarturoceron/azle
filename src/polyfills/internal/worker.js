...
class Worker extends EventEmitter {
    constructor(filename, options = kEmptyObject) {
        ...
        this.performance = {
            eventLoopUtilization: FunctionPrototypeBind(
    ...
    [kOnMessage](message) {
        switch (message.type) {
            ...
            case messageTypes.STDIO_WANTS_MORE_DATA: {
                ...
                return this[kParentSideStdio][stream][
                    kStdioWantsMoreDataCallback
                ]();
            }
        }
    ...
    [kDispose]() {
        ...
        if (!stdout.readableEnded) {
            debug(
                `[${threadId}] explicitly closes stdout for ${this.threadId}`
            );
            stdout.push(null);
        }
