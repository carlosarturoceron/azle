...
const prepareStackTrace = (globalThis, error, trace) => {
    ...
    let lastSourceMap;
    let lastFileName;
    const preparedTrace = ArrayPrototypeJoin(
        ArrayPrototypeMap(trace, (t, i) => {
            const str = i !== 0 ? '\n    at ' : '';
            try {
                ...
                if (sm) {
                    ...
                    // start at 1/1:
                    const { originalLine, originalColumn, originalSource } =
                        sm.findEntry(
                            t.getLineNumber() - 1,
                            t.getColumnNumber() - 1
                        );
                    if (
                        ...
                        const namePrefix =
                            typeName !== null && typeName !== 'global'
                                ? `${typeName}.`
                                : '';
                        const originalName = `${namePrefix}${
                            fnName || '<anonymous>'
                        }`;
...
function getErrorSource(
    ...
    const originalSourcePathNoScheme = StringPrototypeStartsWith(
        originalSourcePath,
        'file://'
    )
    ...
    const exceptionLine = `${originalSourcePathNoScheme}:${
        originalLine + 1
    }\n${line}\n${prefix}^\n\n`;
...
function getOriginalSource(payload, originalSourcePath) {
    ...
    const sourceContentIndex = ArrayPrototypeIndexOf(
        payload.sources,
        originalSourcePath
    );
...
function getSourceMapErrorSource(fileName, lineNumber, columnNumber) {
    ...
    const { originalLine, originalColumn, originalSource } = sm.findEntry(
        lineNumber - 1,
        columnNumber
    );
