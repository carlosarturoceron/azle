 ...
 */
const { async_hook_fields, async_id_fields, execution_async_resources } =
    async_wrap;
...
function defaultTriggerAsyncIdScope(triggerAsyncId, block, ...args) {
    if (triggerAsyncId === undefined) return block.apply(null, args);
...
function pushAsyncContext(asyncId, triggerAsyncId, resource) {
    ...
    async_wrap.async_ids_stack[offset * 2 + 1] =
        async_id_fields[kTriggerAsyncId];
...
function popAsyncContext(asyncId) {
    ...
    async_id_fields[kTriggerAsyncId] =
        async_wrap.async_ids_stack[2 * offset + 1];
    execution_async_resources.pop();
