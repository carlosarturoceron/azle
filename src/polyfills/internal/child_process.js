    ...
    'net.Socket': {
        send(message, socket, options) {
            ...
            if (socket.server) {
                ...
                const firstTime =
                    !this[kChannelHandle].sockets.send[message.key];
...
function ChildProcess() {
    ...
    this._handle.onexit = (exitCode, signalCode) => {
        ...
        if (exitCode < 0) {
            const syscall = this.spawnfile
                ? 'spawn ' + this.spawnfile
                : 'spawn';
...
ChildProcess.prototype.spawn = function (options) {
    ...
    this.stdin =
        stdio.length >= 1 && stdio[0].socket !== undefined
            ? stdio[0].socket
            : null;
    this.stdout =
        stdio.length >= 2 && stdio[1].socket !== undefined
            ? stdio[1].socket
            : null;
    this.stderr =
        stdio.length >= 3 && stdio[2].socket !== undefined
            ? stdio[2].socket
            : null;
...
function setupChannel(target, channel, serializationMode) {
    ...
    const { initMessageChannel, parseChannelMessages, writeChannelMessage } =
        ...
        if (
            ...
            if (target._pendingMessage) {
                if (message.cmd === 'NODE_HANDLE_ACK') {
                    ...
                    closePendingHandle(target);
                    process.emitWarning(
                        'Handle did not reach the receiving process ' +
                            'correctly',
    ...
    target._send = function (message, handle, options, callback) {
        ...
        if (handle) {
            ...
            handle = ReflectApply(handleConversion[message.type].send, target, [
                message,
                handle,
                options
            ]);
...
function spawnSync(options) {
    ...
    if (result.error) {
        result.error = new ErrnoException(
            result.error,
