        ...
        default: {
            class NodeError extends Base {
                ...
                constructor(...args) {
                    ...
                    super(
                        ReflectApply(
                            lazyInternalUtilInspect().format,
                            null,
                            args
                        )
                    );
            ...
            if (otherClasses.length !== 1) {
                otherClasses.forEach((clazz) => {
                    if (clazz !== HideStackFramesError) {
                        ErrClass[clazz.name] = makeNodeErrorWithCode(
                            clazz,
                            sym
                        );
                        ErrClass[clazz.name].HideStackFramesError =
                            makeNodeErrorForHideStackFrame(
                                ErrClass[clazz.name],
                                clazz
                            );
                    }
...
class DNSException extends Error {
    ...
    constructor(code, syscall, hostname) {
        ...
        if (typeof code === 'number') {
            ...
            if (
                code === lazyUv().UV_EAI_NODATA ||
                code === lazyUv().UV_EAI_NONAME
            ) {
...
const fatalExceptionStackEnhancers = {
    ...
    afterInspector(error) {
        ...
        if (isWindows) {
            ...
            const ver = ArrayPrototypeMap(
                StringPrototypeSplit(info[2], '.'),
                Number
            );
...
E(
    ...
    (name, expected, actual) => {
        ...
        if (types.length > 0) {
            msg += `${
                types.length > 1 ? 'one of type' : 'of type'
            } ${formatList(types, 'or')}`;
...
E(
    ...
    (pkgPath, key, target, isImport = false, base = undefined) => {
        ...
        if (key === '.') {
            ...
            return (
                `Invalid "exports" main target ${JSONStringify(
                    target
                )} defined ` +
...
E(
    ...
    (url, supported) => {
        let msg = `Only URLs with a scheme in: ${formatList(
            supported
